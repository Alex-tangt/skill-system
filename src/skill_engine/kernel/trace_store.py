...
class TraceStore:
    """SQLite (WAL mode) storage for execution traces and step traces.

    v0.2: Migrated to kernel. Schema includes LLM context fields.
    """
    ...
    async def initialize(self) -> None:
        async with aiosqlite.connect(self.db_path) as db:
            ...
            # Step 1: Create tables (with all v0.2 columns for new installs)
            await db.execute("""
                CREATE TABLE IF NOT EXISTS execution_traces (
                    ...
                    llm_model TEXT,
                    token_count INTEGER,
                    context_type TEXT DEFAULT 'hook',
                    metadata_json TEXT
                )
            """)
            await db.execute("""
                CREATE TABLE IF NOT EXISTS step_traces (
                    ...
                    event_type TEXT DEFAULT 'step',
                    context_ref TEXT
                )
            """)

            # Step 2: Migrate existing v0.1 databases (add columns if missing)
            await self._migrate_schema(db)

            # Step 3: Create all indexes (safe now that columns exist)
            await db.execute("""
            ...
            """)
            # v0.2 indexes: created AFTER migration ensures columns exist
            await db.execute("""
                CREATE INDEX IF NOT EXISTS idx_traces_context
                ON execution_traces(context_type, parent_run_id)
            """)
            await db.execute("""
                CREATE INDEX IF NOT EXISTS idx_step_traces_event
                ON step_traces(event_type)
            """)

            await db.commit()

    async def _migrate_schema(self, db: aiosqlite.Connection) -> None:
        """Add v0.2 columns if they don't exist (idempotent)."""
        # Check existing columns
        cursor = await db.execute("PRAGMA table_info(execution_traces)")
        ...
        for col, col_type in v02_exec_cols.items():
            if col not in columns:
                await db.execute(
                    f"ALTER TABLE execution_traces ADD COLUMN {col} {col_type}"
                )

        cursor = await db.execute("PRAGMA table_info(step_traces)")
        columns = {row[1] for row in await cursor.fetchall()}
        ...
        for col, col_type in v02_step_cols.items():
            if col not in columns:
                await db.execute(
                    f"ALTER TABLE step_traces ADD COLUMN {col} {col_type}"
                )

        # Track schema version: v1=v0.1, v2=v0.2
        ...
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                """INSERT INTO execution_traces
                   (id, run_id, skill_id, skill_version, started_at, status, input_json,
                    error, parent_run_id, llm_model, token_count, context_type, metadata_json)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    trace.id,
                    trace.run_id,
                    ...
                    trace.token_count,
                    trace.context_type,
                    json.dumps(trace.metadata_json) if trace.metadata_json else None,
                ),
            )
            await db.commit()
            ...
            await db.execute(
                """UPDATE execution_traces
                   SET finished_at = ?, status = ?, output_json = ?, error = ?,
                       llm_model = ?, token_count = ?, context_type = ?, metadata_json = ?
                   WHERE id = ?""",
                (
                    trace.finished_at,
                    trace.status,
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if row:
                await db.execute(
                    """UPDATE step_traces
                       SET finished_at = ?, status = ?, output_json = ?, error = ?,
                           retry_count = ?, event_type = ?, context_ref = ?
                       WHERE id = ?""",
                    (
                        step.finished_at,
                        step.status,
                ...
                await db.execute(
                    """INSERT INTO step_traces
                       (id, trace_id, step_id, step_name, started_at, status, input_json,
                        finished_at, output_json, error, retry_count, event_type, context_ref)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (
                        step.id,
                        step.trace_id,
